Ext.define('TumblrTouch.controller.Main', {
    ...
    goPager: function() {
		...
		console.log(index);
		panel.animateActiveItem(index, {type: 'slide', direction: 'down'});
		panel.setActiveItem(index);
    },
    backPager: function() {
		var panel = Ext.getCmp('cardpanel');
		var index = Number(panel.getActiveItem().id.replace('ext-image-', '')) - 3;
		panel.animateActiveItem(index, {type: 'slide', direction: 'up'});
		panel.setActiveItem(index);
    },
	fetchData: function(){
		...
		ttdata.load({
			callback: function() {
				...
				panel.animateActiveItem(0, {type: 'pop'});
